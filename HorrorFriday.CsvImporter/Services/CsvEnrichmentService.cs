...
""");
    ...
    private async Task<HashSet<int>> LoadDbIdsAsync()
    {
        ...
        await using var conn = await _ds.OpenConnectionAsync();
        await using var cmd = new NpgsqlCommand("SELECT tmdb_id FROM movies WHERE media_type = 'movie'", conn);
    ...
    private static async Task<int> CopyMatchingRowsAsync(NpgsqlConnection conn, string csvPath, HashSet<int> dbIds)
    {
        int count = 0;
        long fileSize = new FileInfo(csvPath).Length;
        await using var writer = await conn.BeginTextImportAsync(
            "COPY enrichment_staging (tmdb_id, budget, revenue, cast_list, director, director_of_photography, " +
            "writers, producers, music_composer, imdb_rating, imdb_votes) FROM STDIN");
        ...
        await foreach (var fields in ParseCsvAsync(csvPath, fileSize, skipHeader: true))
        {
            ...
            count++;
            if (count % 10_000 == 0) Console.Write($"\r  Staging: {count:N0} righe...");
        }
        return count;
    }
    ...
    private static async IAsyncEnumerable<string[]> ParseCsvAsync(string path, long fileSize, bool skipHeader)
    {
        await using var stream = File.OpenRead(path);
        using var reader = new StreamReader(stream, System.Text.Encoding.UTF8, true, 1 << 20);
        var fields = new System.Collections.Generic.List<string>(32);
        ...
        while ((read = await reader.ReadAsync(buf, 0, buf.Length)) > 0)
        {
            ...
            for (int i = 0; i < read; i++)
            {
                ...
                if (inQuotes)
                {
                    if (c == '"') { if (i + 1 < read && buf[i + 1] == '"') { field.Append('"'); i++; } else inQuotes = false; }
                    else field.Append(c);
                }
                else switch (c)
                {
    ...
    private static void AppendText(StringBuilder sb, string? value)
    {
        ...
        foreach (char c in value) switch (c)
        {
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) { sb.Append("\\N"); return; }
        if (double.TryParse(value.Trim(), System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out double d) && d > 0)
            sb.Append((long)d);
        else sb.Append("\\N");
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) { sb.Append("\\N"); return; }
        if (decimal.TryParse(value.Trim(), System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out decimal d) && d > 0)
