...
/// <summary>
/// Imports records that exist only in the new Kaggle CSV and are absent from the DB.
/// Strategy:
///   1. Read diff_only_in_new.txt → target tmdb_id set.
///   2. Load genres table → name-to-id map for genre linking.
///   3. Stream the new CSV, filtering to target IDs only.
///   4. Batch 50 records at a time → OpenAI batch embeddings.
///   5. INSERT each record with all enrichment columns + embedding.
///      ON CONFLICT (tmdb_id, media_type) DO NOTHING keeps it idempotent.
///   6. INSERT movie_genres rows for resolved genre IDs.
/// </summary>
public sealed class NewRecordsImportService : IDisposable
{
    ...
    public NewRecordsImportService(
        string connectionString, string openAiKey,
        string diffFilePath, string newCsvPath)
    {
    ...
    public async Task RunAsync(CancellationToken ct = default)
    {
        ...
        {
            Console.WriteLine("  Nessun ID trovato. Operazione non necessaria.");
            return;
        }
        ...
        // ── Steps 3-6: Stream, batch, embed, insert ───────────────────────────
...
""");
    ...
    private async Task<(int ins, int skip, int err)> FlushBatchAsync(
        NpgsqlConnection conn, List<string[]> batch,
        Dictionary<string, int> genreMap, CancellationToken ct)
    {
        var texts = batch.Select(BuildEmbeddingText).ToList();
        var embeddings = await _embedder.GenerateBatchAsync(texts);
        ...
        for (int i = 0; i < batch.Count; i++)
        {
            ...
            foreach (var gid in genreIds)
                await InsertMovieGenreAsync(conn, dbId.Value, gid);
    ...
    private static async Task<long?> InsertMovieAsync(
        NpgsqlConnection conn, string[] f, int tmdbId, float[] embedding)
    {
        DateOnly? releaseDate = null;
        short? releaseYear = null;
        if (DateOnly.TryParse(f[Col.ReleaseDate], out DateOnly rd))
        {
            releaseDate = rd;
            releaseYear = (short)rd.Year;
        }

        static decimal? ParseDecimal(string s) =>
            decimal.TryParse(s.Trim(), System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture, out decimal v) && v > 0 ? v : null;
        static int? ParseInt(string s) =>
            double.TryParse(s.Trim(), System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture, out double v) && v > 0 ? (int)Math.Round(v) : null;
        static long? ParseLong(string s) =>
            double.TryParse(s.Trim(), System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture, out double v) && v > 0 ? (long)v : null;
        static short? ParseShort(string s) =>
            double.TryParse(s.Trim(), System.Globalization.NumberStyles.Float,
                System.Globalization.CultureInfo.InvariantCulture, out double v) && v > 0 ? (short)Math.Round(v) : null;
    ...
    private static async Task InsertMovieGenreAsync(NpgsqlConnection conn, long movieId, int genreId)
    {
        await using var cmd = new NpgsqlCommand(
            "INSERT INTO movie_genres (movie_id, genre_id) VALUES (@m, @g) ON CONFLICT DO NOTHING",
            conn);
    ...
    // ─── CSV parser (same RFC 4180 logic as CsvEnrichmentService) ────────────
    ...
    private static async IAsyncEnumerable<string[]> ParseCsvAsync(string path, long fileSize)
    {
        ...
        while ((read = await reader.ReadAsync(buf, 0, buf.Length)) > 0)
        {
            ...
            for (int i = 0; i < read; i++)
            {
                ...
                else
                {
                    switch (c)
                    {
                        ...
                        case ',':
                            fields.Add(field.ToString()); field.Clear(); break;
                        case '\n':
            ...
            if (bytesRead - prevReport >= 200 * 1024 * 1024)
            {
                prevReport = bytesRead;
                Console.Write($"\r  Lettura CSV: {(double)bytesRead / fileSize * 100:F0}%...   ");
            }
        }
